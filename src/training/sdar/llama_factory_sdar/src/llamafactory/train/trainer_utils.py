...
def _create_mrp_proj_lr_optimizer(
    model: "PreTrainedModel",
    training_args: "TrainingArguments",
    finetuning_args: "FinetuningArguments",
) -> "torch.optim.Optimizer":
    """Create optimizer with separate LR for MRP projection/norm layers vs transformer layers."""
    proj_lr = finetuning_args.mrp_proj_lr
    default_lr = training_args.learning_rate

    # MRP projection + norm module names
    MRP_PROJ_NORM_KEYS = {"eh_proj", "enorm", "hnorm"}

    decay_param_names = _get_decay_parameter_names(model)

    param_groups_dict: dict[str, list] = {
        "proj_decay": [],
        "proj_nodecay": [],
        "other_decay": [],
        "other_nodecay": [],
    ...
    for name, param in model.named_parameters():
        ...
        is_proj_norm = any(k in name for k in MRP_PROJ_NORM_KEYS)
        # Final norm (mrp_modules.N.model.norm) is NOT in the transformer layers,
        # so include it in the proj group too
        if "mrp_modules." in name and ".model.norm." in name and ".layers." not in name:
            is_proj_norm = True

        is_decay = name in decay_param_names
        if is_proj_norm:
            param_groups_dict["proj_decay" if is_decay else "proj_nodecay"].append(param)
        else:
            param_groups_dict["other_decay" if is_decay else "other_nodecay"].append(param)

    optim_class, optim_kwargs = Trainer.get_optimizer_cls_and_kwargs(training_args)
    param_groups = [
        dict(params=param_groups_dict["proj_decay"], lr=proj_lr, weight_decay=training_args.weight_decay),
        dict(params=param_groups_dict["proj_nodecay"], lr=proj_lr, weight_decay=0.0),
        dict(params=param_groups_dict["other_decay"], lr=default_lr, weight_decay=training_args.weight_decay),
        dict(params=param_groups_dict["other_nodecay"], lr=default_lr, weight_decay=0.0),
    ]
    # Filter out empty groups
    param_groups = [g for g in param_groups if g["params"]]
    optimizer = optim_class(param_groups, **optim_kwargs)
    logger.info_rank0(
        f"Using MRP proj LR optimizer: proj/norm lr={proj_lr}, layers lr={default_lr}. "
        f"proj_decay={len(param_groups_dict['proj_decay'])}, proj_nodecay={len(param_groups_dict['proj_nodecay'])}, "
        f"other_decay={len(param_groups_dict['other_decay'])}, other_nodecay={len(param_groups_dict['other_nodecay'])}."
    )
    return optimizer
...
def create_custom_optimizer(
    ...
    if getattr(finetuning_args, "mrp_proj_lr", None) is not None:
        return _create_mrp_proj_lr_optimizer(model, training_args, finetuning_args)
