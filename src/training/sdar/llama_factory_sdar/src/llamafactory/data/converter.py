...
class SharegptDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        for turn_idx, message in enumerate(messages):
            if message[self.dataset_attr.role_tag] not in accept_tags[turn_idx % 2]:
                logger.warning_rank0(
                    "Invalid role tag '%s' at turn %s in example with %s messages.",
                    message[self.dataset_attr.role_tag],
                    turn_idx,
                    len(messages),
                )
                broken_data = True
        ...
        if (not self.dataset_attr.ranking and len(aligned_messages) % 2 != 0) or (
            ...
            logger.warning_rank0("Invalid message count after alignment: %s", len(aligned_messages))
            broken_data = True
        ...
        if broken_data:
            ...
            if (
                ...
                logger.warning_rank0("Invalid role tag in pairwise chosen/rejected response.")
                broken_data = True
