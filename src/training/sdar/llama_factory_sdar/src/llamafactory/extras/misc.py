...
def calculate_tps(dataset: list[dict[str, Any]], metrics: dict[str, float], stage: Literal["sft", "rm"]) -> float:
    ...
    # Recent transformers versions do not include "epoch" in TrainOutput.metrics;
    # fall back to 1.0 so callers enabling include_effective_tokens_per_second don't crash.
    epoch = metrics.get("epoch", 1.0)
    result = effective_token_num * epoch / metrics["train_runtime"]
